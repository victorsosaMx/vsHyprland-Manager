...
- **Appearance** — General (gaps, border size, rounding, layout), Decoration (active/inactive border colors, opacity), Shadow (enable, range, color, offset), Blur (enable, size, passes, noise, contrast, brightness, vibrancy), XWayland (force zero scaling); wallpaper managed via desktop tool (e.g. swww)
...
- **Scripts** — built-in editor for auxiliary scripts (`hotcorner.sh`, `swww-random.sh`, etc.); Save (with automatic backup), Reload, Load Default and Run buttons; execute-permission toggle (`chmod +x`)
